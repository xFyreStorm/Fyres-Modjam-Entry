        ...
        if (ModjamMod.spawnRandomPillars && world.provider.dimensionId == 0 && random.nextInt(ModjamMod.pillarGenChance) == 0)
        {
            ...
            for (int y = 127, added = 0; y > 30 && !placed && added < max; y--)
            {
                for (int x = chunkX * 16; x < chunkX * 16 + 16 && !placed && added < max; x++)
                {
                    ...
                    {
                        if (random.nextInt(15) != 0 || world.isAirBlock(x, y, z) || Block.blocksList[world.getBlockId(x, y, z)].isBlockReplaceable(world, x, y, z) || world.getBlockId(x, y, z) == ModjamMod.blockTrap.blockID || world.getBlockId(x, y, z) == Block.leaves.blockID)
                        {continue;}
