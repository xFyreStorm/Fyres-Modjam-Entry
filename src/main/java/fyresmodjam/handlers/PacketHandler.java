            ...
            if (side == Side.SERVER)
            {
                ...
                switch (type)
                {
                    ...
                    case ACTIVATE_BLESSING:
                        ...
                        {
                            PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE, new Object[] {"\u00A7cBlessing is on cooldown. (" + (Integer.parseInt(EntityStatHelper.getStat(player, "BlessingCooldown")) - player.worldObj.getWorldTime()) / 20 + "s)"}),
                                    (Player) player);
                        }
                    ...
                    case DISARM_TRAP:
                        ...
                        boolean yours = (!(te == null || !(te instanceof TileEntityTrap))) && player.getEntityName().equals(((TileEntityTrap) te).placedBy);
                        ...
                        if (yours || (mechanic ? ModjamMod.r.nextInt(4) != 0 : ModjamMod.r.nextInt(4) == 0))
                        {
                            ...
                            if (salvage)
                            {
                                player.worldObj.spawnEntityInWorld(new EntityItem(player.worldObj, blockX + 0.5F, blockY, blockZ + 0.5F, new ItemStack(ModjamMod.itemTrap.itemID, 1, player.worldObj.getBlockMetadata(blockX, blockY, blockZ) % BlockTrap.trapTypes)));
                            }
                    ...
                    case EXAMINE_MOB:
                        ...
                        if (server != null)
                        {
                            ...
                            if (entity != null)
                            {
                                ...
                                {
                                    PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE, new Object[] {"\u00A7eThere doesn't seem to be anything special about " + (entity instanceof EntityPlayer ? "" : "this ") + entity.getTranslatedEntityName() + "\u00A7e."}),
                                            (Player) player);
                                }
                            }
                        }
            ...
            else if (side == Side.CLIENT)
            {
                ...
                switch (type)
                {
                    case UPDATE_BLESSING:
                        player.getEntityData().setString("Blessing", inputStream.readUTF());
                        return;
                    case UPDATE_POTION_KNOWLEDGE:
                        int[] potionKnowledge = new int[12];
                        for (int i = 0; i < 12; i++) {potionKnowledge[i] = inputStream.readInt();}
                        player.getEntityData().setIntArray("PotionKnowledge", potionKnowledge);
                        return;
                    ...
                    case LEVEL_UP:
                        player.addExperienceLevel(inputStream.readInt());
                        return;
                    ...
                    case UPDATE_WORLD_DATA:
                        ...
                        for (int i = 0; i < 13; i++) for (int i2 = 0; i2 < 2; i2++) {mushroomColors[i][i2] = inputStream.readInt();}
                }
            }
        }
        ...
        try
        {
            ...
            if (data != null)
            {
                for (Object aData : data)
                {
                    ...
                    {outputStream.writeInt((Integer) aData);}
                    else if (aData instanceof int[])
                    {
                        for (int i2 = 0; i2 < ((int[]) aData).length; i2++) {outputStream.writeInt(((int[]) aData)[i2]);}
                    }
                    else if (aData instanceof int[][])
                    {
                    ...
                    {outputStream.writeBoolean((Boolean) aData);}
                    else if (aData instanceof String)
                    {outputStream.writeUTF((String) aData);}
                    else if (aData instanceof Byte)
                    {outputStream.writeByte((Byte) aData);}
                    else if (aData instanceof Float)
                    {outputStream.writeFloat((Float) aData);}
                    else if (aData instanceof Double)
                    {outputStream.writeDouble((Double) aData);}
                    else if (aData instanceof Character) {outputStream.writeChar((Character) aData);}
                }
            }
        }
