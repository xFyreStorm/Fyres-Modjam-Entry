...
public class ItemStatHelper implements ICraftingHandler
{
    ...
    public static void addStatTracker(ItemStatTracker statTracker)
    {
        ...
        if (statTracker.ids != null)
        {
            for (int i : statTracker.ids)
            {
                if (i < 0) {continue;}
                statTrackersByID.put(i, statTracker);
            }
        }
    ...
    public static void processItemStack(ItemStack stack, Random r)
    {
        ...
        for (ItemStatTracker e : genericTrackers)
        {
            if (!temp.contains(e))
            {
                ...
                {
                    if (c.isAssignableFrom(stack.getItem().getClass()))
                    {
                        temp.add(e);
                        break;
                    }
                ...
                for (int i = 0; i < EntityStatHelper.knowledge.length; i++)
                {
                    if (EntityStatHelper.killCount[i] == craftingStats.getInteger(itemName))
                    {
                        PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE,
                                new Object[] {"\u00A7o\u00A73You've become a " + EntityStatHelper.knowledge[i].toLowerCase() + " " + itemName.toLowerCase() + " smith! (" + (i < EntityStatHelper.knowledge.length - 1 ? (EntityStatHelper.killCount[i + 1] * 2 - EntityStatHelper.killCount[i] * 2) + " " + itemName.toLowerCase() + " crafts to next rank." : "")}),
                                (Player) player);
                        break;
                    }
                }
