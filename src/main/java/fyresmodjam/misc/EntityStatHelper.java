...
public class EntityStatHelper
{
    ...
    public void entityJoinWorld(EntityJoinWorldEvent event)
    {
        if (!event.world.isRemote)
        {
            ...
            isClone = event.entity.getEntityData().hasKey("isClone") && event.entity.getEntityData().getBoolean("isClone");
    ...
    public void livingDeath(LivingDeathEvent event)
    {
        if (!event.entity.worldObj.isRemote)
        {
            ...
            if (CommonTickHandler.worldData.currentTask.equals("Kill") && FyresWorldData.validMobs[CommonTickHandler.worldData.currentTaskID].isAssignableFrom(event.entity.getClass()))
            {
                ...
                PacketDispatcher.sendPacketToAllPlayers(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE,
                        new Object[] {"\u00A7fCurrent Goal Progress: " + CommonTickHandler.worldData.progress + "/" + CommonTickHandler.worldData.currentTaskAmount + " " + name1 + " " + CommonTickHandler.worldData.currentTask + "ed."}));
                ...
                if (CommonTickHandler.worldData.progress >= CommonTickHandler.worldData.currentTaskAmount)
                {
                    ...
                    PacketDispatcher.sendPacketToAllPlayers(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE,
                            new Object[] {"\u00A7eA new world goal has been set: " + (CommonTickHandler.worldData.currentTask + " " + CommonTickHandler.worldData.currentTaskAmount + " " + (CommonTickHandler.worldData.currentTask.equals("Kill") ? FyresWorldData.validMobNames[CommonTickHandler.worldData.currentTaskID] : new ItemStack(
                                    Item.itemsList[CommonTickHandler.worldData.currentTaskID],
                                    1).getDisplayName()) + "s. (" + CommonTickHandler.worldData.progress + " " + CommonTickHandler.worldData.currentTask + "ed)")}));
        ...
        if (event.entity instanceof EntityPlayer)
        {
            ...
            {
                CommonTickHandler.worldData.killStatsByPlayer.put(player.getEntityName(), player.getEntityData().getCompoundTag("KillStats"));
            }
            if (player.getEntityData() != null && player.getEntityData().hasKey("WeaponStats"))
            {
                CommonTickHandler.worldData.weaponStatsByPlayer.put(player.getEntityName(), player.getEntityData().getCompoundTag("WeaponStats"));
            }
            if (player.getEntityData() != null && player.getEntityData().hasKey("CraftingStats"))
            {
                CommonTickHandler.worldData.craftingStatsByPlayer.put(player.getEntityName(), player.getEntityData().getCompoundTag("CraftingStats"));
            }
        ...
        else if (event.source != null && event.source.getEntity() != null && event.source.getEntity() instanceof EntityPlayer)
        {
            ...
            if (ModjamMod.enableMobKillStats)
            {
                ...
                {
                    if (killCount[i] == killStats.getInteger(mob + "Kills"))
                    {
                        PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE,
                                new Object[] {"\u00A7o\u00A73You've become a " + knowledge[i].toLowerCase() + " " + mob.toLowerCase() + " slayer! (+" + damageBonusString[i] + "% damage against " + mob.toLowerCase() + "s.)" + (i < knowledge.length - 1 ? " " + (killCount[i + 1] - killCount[i]) + " " + mob.toLowerCase() + " kills to next rank." : "")}),
                                (Player) player);
                        break;
                    }
                }
            ...
            if (ModjamMod.enableWeaponKillStats)
            {
                for (int i = 0; i < knowledge.length; i++)
                {
                    if (killCount[i] * 2 == weaponStats.getInteger(weapon + "Kills"))
                    {
                        PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE,
                                new Object[] {"\u00A7o\u00A73You've become a " + knowledge[i].toLowerCase() + " " + weapon.toLowerCase() + " user! (+" + damageBonusString[i] + "% damage with " + weapon.toLowerCase() + "s.)" + (i < knowledge.length - 1 ? " " + (killCount[i + 1] * 2 - killCount[i] * 2) + " " + weapon.toLowerCase() + " kills to next rank." : "")}),
                                (Player) player);
                        break;
                    }
                }
