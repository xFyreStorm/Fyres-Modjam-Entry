...
public class TileEntityTrap extends TileEntity
{
    ...
    public double getMaxRenderDistanceSquared()
    {
        ...
        //EntityPlayer player = Minecraft.getMinecraft().thePlayer;
        return (player != null && (player.getEntityName().equals(placedBy) || setting != 0)) ? 4096.0F : 36.0F; //(player != null && player.getEntityData().hasKey("Blessing") && player.getEntityData().getString("Blessing").equals("Scout")) ? 16.0D : 36.0D;
    }
    ...
    public void spawnParticles()
    {
        ...
        if (player != null && (!PacketHandler.trapsDisabled || placedBy != null) && (player.getEntityName().equals(placedBy) || player.isSneaking() || setting != 0 || (player.getEntityData().hasKey("Blessing") && player.getEntityData().getString("Blessing").equals("Scout"))) && this.getDistanceFrom(
                TileEntityRenderer.staticPlayerX,
                TileEntityRenderer.staticPlayerY,
                TileEntityRenderer.staticPlayerZ) < (player.getEntityName().equals(placedBy) ? 4096 : 36.0F))
        {
            if (type == 1)
            {
                if (ModjamMod.r.nextInt(5) == 0)
                {
                    this.worldObj.spawnParticle("smoke", this.xCoord + 0.5F, this.yCoord + 0.175F, this.zCoord + 0.5F, (ModjamMod.r.nextFloat() - 0.5F) / 16, ModjamMod.r.nextFloat() / 16, (ModjamMod.r.nextFloat() - 0.5F) / 16);
                }
