...
public class ModjamMod extends CommandHandler implements IPlayerTracker
{
    ...
    public static boolean pillarGlow = true, spawnTraps = true, spawnTowers = true, spawnRandomPillars = true, disableDisadvantages = false, versionChecking = true, trapsBelowGroundOnly = false, showAllPillarsInCreative = false, enableWeaponKillStats = true, enableMobKillStats = true, enableCraftingStats = true, enableMobNaming = true;
    ...
    public void preInit(FMLPreInitializationEvent event)
    {
        ...
        if (versionChecking)
        {
            ...
            if (!version.equals(foundVersion) && ((Integer.parseInt(versionSplit[0]) < Integer.parseInt(foundSplit[0])) && ((Integer.parseInt(versionSplit[1]) < Integer.parseInt(foundSplit[1])) && ((Integer.parseInt("" + versionSplit[2].charAt(0)) < Integer.parseInt("" + foundSplit[2].charAt(0))) && (versionOrder.indexOf(
                    versionSplit[2].charAt(1)) < versionOrder.indexOf(foundSplit[2].charAt(1)))))))
            {
    ...
    public void init(FMLInitializationEvent event)
    {
        ...
        weaponTracker.addStat(new ItemStat("Rank", "")
        {
            ...
            public String[][] prefixesByRank = {{"Old", "Dull", "Broken", "Worn"}, {"Average", "Decent", "Modest", "Ordinary"}, {"Strong", "Sharp", "Polished", "Refined"}, {"Powerful", "Ruthless", "Elite", "Astonishing"}, {"Godly", "Divine", "Fabled", "Legendary"}};
        ...
        armorTracker.addStat(new ItemStat("Rank", "")
        {
            ...
            public String[][] prefixesByRank = {{"Old", "Broken", "Worn", "Weak"}, {"Average", "Decent", "Modest", "Ordinary"}, {"Polished", "Tough", "Hardened", "Durable"}, {"Elite", "Astonishing", "Reinforced", "Resilient"}, {"Godly", "Divine", "Fabled", "Legendary"}};
    ...
    public void onPlayerLogin(EntityPlayer player)
    {
        ...
        {
            PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.UPDATE_WORLD_DATA,
            ...
            PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE, new Object[] {"\u00A7eWorld disadvantage: " + CommonTickHandler.worldData.getDisadvantage() + (index == -1 ? "" : " (" + CommonTickHandler.worldData.disadvantageDescriptions[index] + ")")}),
                    (Player) player);

            PacketDispatcher.sendPacketToPlayer(PacketHandler.newPacket(PacketHandler.SEND_MESSAGE,
                    new Object[] {"\u00A7eWorld goal: " + CommonTickHandler.worldData.currentTask + " " + CommonTickHandler.worldData.currentTaskAmount + " " + name + ". (" + CommonTickHandler.worldData.progress + " " + CommonTickHandler.worldData.currentTask + "ed)"}), (Player) player);
