			...
			if(inputStream != null) {
				...
				if (side == Side.SERVER) {
					...
					switch(type) {
						...
						case UPDATE_WORLD_DATA: CommonTickHandler.worldData.checkWorldData(); for(int i = 0; i < 12; i++) {FyresWorldData.potionValues[i] = inputStream.readInt();} for(int i = 0; i < 12; i++) {FyresWorldData.potionDurations[i] = inputStream.readInt();} FyresWorldData.currentDisadvantage = inputStream.readUTF(); return;
						default: return;
					}
				}
			}
